...
def send_ntfy_notification(new_items):
    ...
    for item in to_send:
        ...
        # DIAGNOSTIC TEMPORAIRE : payload réduit au strict minimum (juste
        # topic + message), pour isoler quel champ du payload complet
        # empêchait la livraison Firebase malgré un 200 OK du serveur ntfy.
        # Une fois la cause confirmée, réintroduire title/tags/click/attach
        # progressivement.
        payload = {
            "topic": NTFY_TOPIC,
            "message": (("[OFFICIEL] " if item.get("official") else "") + item["source"] + " — " + item["title"])[:250],
        }
        ...
        try:
            ...
            if resp.status_code == 200:
                print(f"  [ntfy] envoyé (payload minimal) : {item['title'][:60]}")
            else:
