...
def send_ntfy_notification(new_items):
    """Envoie une vraie notification push Android/iOS via ntfy.sh.

    DÉSACTIVÉE dans main() : le serveur ntfy.sh confirmait l'envoi (200 OK)
    mais ne relayait jamais les messages jusqu'au téléphone, même avec un
    payload minimal (juste topic + message). Cause probable : fail2ban ou
    rate-limit silencieux sur les IP partagées des runners GitHub Actions
    (documenté par ntfy lui-même : "there's even a fail2ban in place which
    will ban your IP pretty quickly"). Fonction gardée intacte si jamais
    utile depuis un contexte avec une IP plus stable (ex: exécution locale).
    """
    if not NTFY_TOPIC:
...
def main():
    ...
    if not is_first_run:
        ...
        # ntfy désactivé : le serveur ntfy.sh acceptait les requêtes (200 OK)
        # mais ne relayait jamais les messages au téléphone, probablement à
        # cause d'un fail2ban ou d'un rate-limit silencieux sur les adresses
        # IP partagées des runners GitHub Actions (confirmé non résolu après
        # plusieurs tentatives, y compris avec un payload minimal). Discord
        # utilise un simple webhook HTTP sans cette dépendance à Firebase.
