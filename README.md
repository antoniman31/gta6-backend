...
Veille automatisée de l'actualité GTA 6 : un robot interroge 52 sources en
...
2. **Récupère les 52 sources** (liste `FEEDS`) **en parallèle**, avec
...
4. **Écarte les archives** — un article jamais vu dont la date dépasse
...
5. **Décode les liens Google News** — ces flux renvoient normalement des
...
6. **Récupère les miniatures manquantes** en parallèle (`IMAGE_WORKERS`,
...
7. **Compte les sources et déduplique** — quand plusieurs rédactions
...
8. **Plafonne l'historique à 20 000 articles** (`MAX_HISTORY_SIZE`) — au-delà,
   ...
   fichier.
9. **Dépose les nouveaux articles** dans le fichier désigné par
   ...
   nouveau).
10. **Écrit aussi `docs/feed-recent.json`** — les 300 articles les plus
   ...
   fusion de conflit.
11. **Dresse l'état de chaque source** (`sources_health` dans `feed.json`) —
...
12. **Écrit `docs/feed.json`** avec l'historique complet, les métadonnées
